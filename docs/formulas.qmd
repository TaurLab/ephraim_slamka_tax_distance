...
```{r}
values <- c("Euclidean"="$\\mathrm{Deuc}_{jk} = \\sqrt{\\sum_i (x_{ij}-x_{ik})^2}$",
            "Manhattan"="$\\mathrm{Dman}_{jk}=\\sum_i \\mid x_{ij}-x_{ik} \\mid$",
            "Morisita"="$\\mathrm{Dmor}_{jk} =  1 - \\dfrac{2 \\sum_i x_{ij} x_{ik}}{(\\lambda_j +
              \\lambda_k) \\sum_i x_{ij} \\sum_i
              x_{ik}}$, where $\\lambda_j = \\dfrac{\\sum_i x_{ij} (x_{ij} - 1)}{\\sum_i x_{ij} \\sum_i (x_{ij} - 1)}$",
            "Morisita-Horn"="$\\mathrm{Dhorn}_{jk} =  1 - \\dfrac{2 \\sum_i x_{ij} x_{ik}}{(\\lambda_j +
              \\lambda_k) \\sum_i x_{ij} \\sum_i
...
```{r}
values2 <- c("Unweighted Unifrac"="$\\mathrm{Dunifrac}_{jk} = \\dfrac{\\sum_n l_n \\mid A_n - B_n \\mid}{\\sum_n l_n \\max(X_{nj}, X_{nk})}$")
...
We used Horn index to calculate $\mathrm{Dhorn}_{jk}$, representing the Horn calculation across all taxonomic levels
