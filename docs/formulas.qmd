...
values <- c("Euclidean"="$Deuc_{jk} = \\sqrt{\\sum_i (x_{ij}-x_{ik})^2}$",
            ...
            "Morisita"="$Dmor_{jk} =  1 - \\frac{2 \\sum_i x_{ij} x_{ik}}{(\\lambda_j +
              \\lambda_k) \\sum_i x_{ij} \\sum_i
              x_{ik}}$, where $\\lambda_j = \\frac{\\sum_i x_{ij} (x_{ij} - 1)}{\\sum_i x_{ij} \\sum_i (x_{ij} - 1)}$",
            "Morisita-Horn"="$Dhorn_{jk} =  1 - \\frac{2 \\sum_i x_{ij} x_{ik}}{(\\lambda_j +
              \\lambda_k) \\sum_i x_{ij} \\sum_i
...
```{r}
values2 <- c("Unweighted Unifrac"="$Dunifrac_{jk} = \\frac{\\sum_n l_n \\mid A_n - B_n \\mid}{\\sum_n l_n \\max(X_{nj}, X_{nk})}$")
...
We used Horn index to calculate $Dhorn_{jk}$, representing the Horn calculation across all taxonomic levels
